...
export default function Technology({ technologies }: TechnologyProps) {
  ...
  return (
    <section id="tech" className="py-24 sm:py-32 px-6 sm:px-8 max-w-7xl mx-auto border-t border-white/[0.06]">
      {/* Section Header */}
        ...
        {categories.map((category, catIdx) => {
          ...
          return (
            <motion.div
              ...
              className="rounded-3xl bg-surface-card border border-white/[0.08] p-8 hover:border-white/20 transition-all duration-300 flex flex-col justify-between"
            >
              <div>
                ...
                <div className="space-y-4">
                  {items.map((tech) => (
                    <div
                      key={tech.id}
                      className="group/item flex flex-col sm:flex-row sm:items-baseline justify-between gap-1 p-3 rounded-xl hover:bg-white/[0.03] transition-colors"
                    >
                      <div className="flex items-center gap-3">
                        <span className="w-1.5 h-1.5 rounded-full bg-crimson-500/60 group-hover/item:bg-crimson-500 transition-colors" />
                        <span className="text-base sm:text-lg font-bold text-white font-mono tracking-tight">
                          {tech.name}
                        </span>
                      </div>
                      <span className="text-xs font-mono text-text-muted pl-4 sm:pl-0 truncate max-w-xs">
                        {tech.focus}
                      </span>
                    </div>
                  ))}
