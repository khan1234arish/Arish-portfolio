...
export default function Process({ steps }: ProcessProps) {
  return (
    <section id="process" className="py-20 sm:py-32 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto border-t border-white/[0.06]">
      {/* Section Title */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {steps.map((step, idx) => (
          <motion.div
            ...
            className="group relative rounded-3xl bg-surface-card border border-white/[0.08] p-5 sm:p-7 hover:border-crimson-500/40 transition-all duration-300 flex flex-col justify-between"
          >
            <div>
              {/* Step indicator */}
              <div className="flex items-center justify-between pb-4 sm:pb-6 mb-4 sm:mb-6 border-b border-white/[0.06]">
                <span className="font-mono text-xs font-bold px-2.5 py-1 rounded-md bg-white/[0.04] text-crimson-400 border border-white/[0.06]">
                  PHASE {step.step}
                </span>
              ...
              {/* Title & Subtitle */}
              <h3 className="text-lg sm:text-xl font-bold tracking-tight text-white group-hover:text-crimson-400 transition-colors">
                {step.title}
              </h3>
              <p className="text-xs font-mono text-text-muted mt-1 mb-3 sm:mb-4">
                {step.subtitle}
              </p>
            ...
            {step.deliverables && step.deliverables.length > 0 && (
              <div className="pt-4 sm:pt-6 mt-4 sm:mt-6 border-t border-white/[0.06] space-y-1.5 sm:space-y-2">
                ...
                {step.deliverables.map((item, itemIdx) => (
                  <div key={itemIdx} className="text-[11px] font-mono text-text-secondary flex items-center gap-2">
                    <span className="w-1 h-1 rounded-full bg-crimson-500 shrink-0" />
                    <span>{item}</span>
                  </div>
                ))}
              </div>
